...
export class ProductTraceabilityComponent implements OnInit {
  ...
  load(referenceNumber:string){
    this.traceabilityService.getTrace(referenceNumber).subscribe(
      (data) => {
        this.produto = data
        console.log("teste")
  ...
  createGraph(){
    ...
    this.nodes.push({
      id: id,
      label: ["Quantidade Disponivel",this.produto.availableQuantity.toString()].join(' : '),
      data :{
  ...
  createNode(parent:string,product:Produto){
    ...
    this.nodes.push({
      id: id,
      label: ["Quantidade Utilizada",product.usedQuantityAsInput.toString()].join(' : '),
      data :{
